...
function maskBadge(instance) {
  return (
    ...
    escHtml(instance.clientIdMasked) +
    "</span>"
  );
}
...
function renderInstances() {
  ...
  if (instances.length === 0) {
    list.innerHTML =
      '<p style="color:var(--text-muted);font-size:13px;margin-bottom:14px;">No instances added yet.</p>';
    return;
  }
  list.innerHTML = instances
    .map(function (inst) {
      return (
        '<div class="instance-row" id="inst-' +
        inst.id +
        '">' +
...
document
  .getElementById("add-instance-form")
  .addEventListener("submit", async function (e) {
    ...
    if (!res.ok) {
      showAlert(
        ...
        data.error || "Failed to add instance",
      );
      return;
    }
    showAlert(
      document.getElementById("add-success"),
      'Instance "' + data.name + '" added.',
      "success",
    );
    e.target.reset();
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    const res = await apiFetch("/api/actions/" + iid + "/end-users", {
      method: "POST",
      body: JSON.stringify(body),
    });
    if (!res) return;
    ...
    if (!res.ok) {
      showAlert(
        document.getElementById("eu-create-error"),
...
// ===== Technicians =====
document
  .getElementById("create-tech-form")
  .addEventListener("submit", async function (e) {
    e.preventDefault();
    const iid = getActiveInstance();
    if (!iid) {
    ...
    if (!body.roleName) delete body.roleName;
    const res = await apiFetch("/api/actions/" + iid + "/technicians", {
      method: "POST",
      body: JSON.stringify(body),
    });
    if (!res) return;
    ...
    if (!res.ok) {
      showAlert(
        document.getElementById("tech-create-error"),
        data.error || "Failed to create technician",
      );
      return;
    }
...
document
  ...
  .addEventListener("click", async function () {
    ...
    el.innerHTML = "";
    if (!res.ok) {
      el.appendChild(jsonBlock({ error: data.error }));
      return;
    }
    if (Array.isArray(data) && data.length > 0) {
      ...
      table.innerHTML =
        "<thead><tr><th>ID</th><th>Hostname</th></tr></thead><tbody>" +
        data
          .map(function (d) {
            return (
              "<tr><td>" +
      ...
      el.innerHTML =
        '<p style="color:var(--text-muted);font-size:13px;">No devices found.</p>';
    }
  });

// ===== Device-User Report =====
document
  .getElementById("report-view-btn")
  .addEventListener("click", async function () {
    const iid = getActiveInstance();
    if (!iid) {
      alert("Select an instance first.");
    ...
    table.className = "data-table";
    table.innerHTML =
      "<thead><tr><th>Device ID</th><th>Hostname</th><th>OS</th><th>User ID</th><th>First</th><th>Last</th><th>Email</th></tr></thead><tbody>" +
      data
        .map(function (r) {
          return (
            "<tr>" +
            "<td>" +
            escHtml(String(r.deviceId)) +
            "</td>" +
            '<td style="' +
            (r.hostname === "(device not found)" ? "color:var(--danger)" : "") +
            '">' +
            escHtml(r.hostname) +
            "</td>" +
            "<td>" +
...
document
  ...
  .addEventListener("click", function () {
    ...
    window.location.href =
      "/api/actions/" + iid + "/device-user-mapping?format=csv";
  });
